...
class FastSpeech2Accent(AbsTTS):
    """FastSpeech2 module.
    ...
    def forward(
        ...
        before_outs, after_outs, d_outs, p_outs, e_outs = self._forward(
            phoneme_xs, accent_xs, ilens, ys, olens, ds, ps, es, spembs=spembs, is_inference=False
        )
    ...
    def _forward(
        ...
        hs, _ = self.encoder(phoneme_xs, accent_xs, x_masks)  # (B, Tmax, adim)
        ...
        d_masks = make_pad_mask(ilens).to(phoneme_xs.device)
    ...
    def inference(
        ...
        # setup batch axis
        ilens = torch.tensor([phoneme.shape[0]], dtype=torch.long, device=phoneme.device)
        phoneme_xs, accent_xs, ys = phoneme.unsqueeze(0), accent.unsqueeze(0), None
