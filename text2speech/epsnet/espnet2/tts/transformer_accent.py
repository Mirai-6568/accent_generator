...
from espnet2.tts.encoder_accent import EncoderAccent


class TransformerAccent(AbsTTS):
    ...
    Args:
        idim (int): Dimension of the phoneme.
        accent_dim (int): Dimension of the accent.
        odim (int): Dimension of the outputs.
        phoneme_embed_dim (int): Dimension of phoneme embedding.
        accent_embed_dim (int): Dimension of accent embedding.
    ...
    def __init__(
        ...
        accent_dim: int,
        odim: int,
        phoneme_embed_dim: int = 192,
        accent_embed_dim: int = 64,
        ...
        self.phoneme_dim = idim
        self.accent_dim = accent_dim
        self.odim = odim
        self.eos = self.phoneme_dim - 1
        self.accent_eos = accent_dim - 1
        ...
        # accent_embed_dim + phoneme_embed_dim should be equaled to adim.
        assert phoneme_embed_dim + accent_embed_dim == adim, \
            'phoneme_embed_dim + accent_embed_dim should be equaled to adim.'
        self.encoder = EncoderAccent(
            attention_dim=adim,
            phoneme_dim=self.phoneme_dim,
            accent_dim=accent_dim,
            phoneme_embed_dim=phoneme_embed_dim,
            accent_embed_dim=accent_embed_dim,
    ...
    def _reset_parameters(self, init_type, init_enc_alpha=1.0, init_dec_alpha=1.0):
        ...
        if self.use_scaled_pos_enc:
            self.encoder.pos_enc_class.alpha.data = torch.tensor(init_enc_alpha)
    ...
    def forward(
        ...
        Args:
            text (LongTensor): Batch of padded phoneme ids (B, Tmax).
            accent (LongTensor): Batch of padded accent ids (B, Tmax).
            text_lengths (LongTensor): Batch of lengths of each input batch (B,).
            accent_lengths (LongTensor): Batch of lengths of each input batch (B,).
        ...
        accent = accent[:, : text_lengths.max()]  # for data-parallel
        speech = speech[:, : speech_lengths.max()]  # for data-parallel
        batch_size = text.size(0)

        # Add eos at the last of sequence
        phoneme_xs = F.pad(text, [0, 1], "constant", self.padding_idx)
        accent_xs = F.pad(accent, [0, 1], "constant", self.padding_idx)

        for i, l in enumerate(text_lengths):
            phoneme_xs[i, l] = self.eos
            accent_xs[i, l] = self.accent_eos
        ...
        after_outs, before_outs, logits = self._forward(phoneme_xs, accent_xs, ilens, ys, olens, spembs)
        ...
        if self.use_guided_attn_loss:
            ...
            if "encoder-decoder" in self.modules_applied_guided_attn:
                ...
                focus_rates = []
                for b in att_ws:
                    _, focus_rate = duration_calculator(torch.unsqueeze(b, 0)) 
                    focus_rates.append(float(focus_rate))
                average_focus_rate = float(np.average(np.array(focus_rates)))
                stats.update(focus_rates=average_focus_rate)
        ...
        if self.use_scaled_pos_enc:
            stats.update(
                encoder_alpha=self.encoder.pos_enc_class.alpha.data.item(),
    ...
    def _forward(
        self,
        phoneme_xs: torch.Tensor,
        accent_xs: torch.Tensor,
        ilens: torch.Tensor,
        ys: torch.Tensor,
        olens: torch.Tensor,
        spembs: torch.Tensor,
        ...
        hs, h_masks = self.encoder(phoneme_xs, accent_xs, x_masks)
    ...
    def inference(
        ...
        Args:
            ...
            accent (LongTensor): Input sequence of accents (T,).
        ...
        # add eos at the last of sequence
        phoneme = F.pad(phoneme, [0, 1], "constant", self.eos)
        accent = F.pad(accent, [0, 1], "constant", self.accent_eos)
        ...
        if use_teacher_forcing:
            ...
            phoneme_xs, accent_xs, ys = phoneme.unsqueeze(0), accent.unsqueeze(0), y.unsqueeze(0)
            spembs = None if spemb is None else spemb.unsqueeze(0)
            ilens = phoneme.new_tensor([phoneme_xs.size(1)]).long()
            olens = y.new_tensor([ys.size(1)]).long()
            outs, *_ = self._forward(phoneme_xs, accent_xs, ilens, ys, olens, spembs)
        ...
        # forward encoder
        phoneme_xs = phoneme.unsqueeze(0)
        accent_xs = accent.unsqueeze(0)
        hs, _ = self.encoder(phoneme_xs, accent_xs, None)
        ...
        z_cache = self.decoder.init_state(phoneme)
        while True:
            # update index
            idx += 1

            # calculate output and stop prob at idx-th step
            y_masks = subsequent_mask(idx).unsqueeze(0).to(phoneme.device)
...
if __name__ == '__main__':
    ...
    t = TransformerAccent(idim=43,adim=256, accent_dim=2, odim=80)
    print(t)
